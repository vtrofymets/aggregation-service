...
public class FlywayConfiguration {
    ...
    public void init() {
        dataSourcesSettings.dataSources()
                .stream()
                .filter(dataSourceProperties -> Optional.ofNullable(dataSourceProperties.migration()).filter(DataSourcesSettings.MigrationProperties::enabled).isPresent())
                .map(dataSourceProperties -> Flyway.configure()
                        .dataSource(DataSourceBuilder.create().url(dataSourceProperties.url()).username(dataSourceProperties.user()).password(dataSourceProperties.password()).build())
